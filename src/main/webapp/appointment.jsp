					...
					<div class="row">
						<div class="page-header">
							<div class="d-flex align-items-center">
								<h2 class="page-header-title">Appointment</h2>
								<div>
									<ul class="breadcrumb">
											</ul>
								</div>
									...
									<form action="AdminServlet">
										<div class="table-responsive">
											<table class="table mb-0">
												...
												<tbody>
													<c:forEach var="ap" items="${users}">

														<tr>

															<td>${ap.patient.patient_name}</td>
															<td>${ap.doctor.dr_name}</td>
															<td>${ap.booking_date}</td>
															<td>${ap.appointment_date}</td>
															<%-- <td>${ap.status}</td> --%>
															<c:if test="${ap.status eq 'accepted'}">
																<td><span style="width: 100px;"><span
																		class="badge-text badge-text-small success">
																			Accepted</span></span></td>
															</c:if>
															<c:if test="${ap.status eq 'denied'}">
																<td><span style="width: 100px;"><span
																		class="badge-text badge-text-small danger">Denied</span></span></td>
															</c:if>
															<c:if test="${ap.status eq 'pending'}">
																<td><span style="width: 100px;"><span
																		class="badge-text badge-text-small info">Pending</span></span></td>
															</c:if>
															<c:if test="${ap.status eq 'visited'}">
																<td><span style="width: 100px;"><span
																		class="badge-text badge-text-small success">Visited</span></span></td>
															</c:if>
														</tr>
													</c:forEach>
