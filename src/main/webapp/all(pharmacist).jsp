					...
					<div class="row">
						<div class="page-header">
							<div class="d-flex align-items-center">
								<h2 class="page-header-title">Pharmacist</h2>
								<div>
									<ul class="breadcrumb">
									</ul>
								</div>
									...
									<form action="AdminServlet">
										<div class="table-responsive">
											<table class="table mb-0">
												...
												<tbody>
													<c:forEach var="ph" items="${users}">

														<tr>
															<td>${ph.phst_id}</td>
															<td>${ph.phst_name}</td>
															<td>${ph.log_email}</td>
															<td>${ph.phst_gender}</td>
															<td>${ph.phst_dob}</td>
															<td>${ph.phst_mobile}</td>
															<td>${ph.address.add_detail},${ph.address.landmark}</br>${ph.address.area},${ph.address.city}-${ph.address.pincode}</td>
															<td><c:choose>
																	<c:when test="${ph.ifd eq 0}">
																		<a
																			href="AdminServlet?action=pharmacist_delete&&ifd=1&&id=${ph.phst_id}">Active</a>
																	</c:when>
																	<c:otherwise>
																		<a
																			href="AdminServlet?action=pharmacist_delete&&ifd=0&&id=${ph.phst_id}">InActive</a>
																	</c:otherwise>
																</c:choose></td>
														</tr>
													</c:forEach>
